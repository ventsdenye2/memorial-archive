...
源文案：剧情纸条0908.docx及后续补充。按本次确认范围接入 16 条可阅读记录、12 段剧情（含阅读后的自言自语）。OpeningStory 未改；旧档案室、禁闭室暂不接入。当前运行配置另外包含新手引导条目。

## 游戏行为

- 纸条、日记和档案互动后自动打开笔记，定位对应分类与条目；左右箭头翻页及切换条目。末页读完并关闭/切换后标记已读。
- 前厅、二楼走廊、主管办公室、会客室、馆长办公室首次进入时显示场景叠加剧情。保留当前场景与 HUD，暂停探索，任意键推进；结束当帧仍拦截探索输入，防止推进点击同时攻击。
- 二楼的“拾级而上……”与“橡木门半掩……”在首次到达 Floor_2F 时强制播放；会客室（文案中的休息室）入场从“我推开二楼休息室的门……”开始，不重复上楼描述。已完成的剧情随存档记录，不会在再次进出房间时重播。
- 二楼探索要求进入档案室 A、档案室 B、会客室，完成 Cecil 对话，并尝试打开上锁的主管办公室；满足后在二楼走廊播放“或许我应该上三楼看看”。办公室钥匙在三楼，进入办公室不再是上楼条件。未满足时每次上楼都提示；详细终点条件见 Docs/PLAY_FLOW_0915.md。
